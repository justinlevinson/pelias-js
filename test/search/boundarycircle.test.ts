...
describe('Boundary Circle', () => {
  ...
  it('Correctly sets a boundary circle', () => {
    ...
    expect(search).toHaveBeenCalledWith("http://127.0.0.1:4000","boundary.circle.lat=0&boundary.circle.lon=0&boundary.circle.radius=1")
  })

  it('Correctly sets an admin area and text', () => {
    const client = new Pelias({peliasUrl: "http://127.0.0.1:4000"})
    client.search.setBoundaryCircle({lat: 0, lon: 0, radius: 1}).setSearchTerm("test").execute()
    expect(search).toHaveBeenCalledWith("http://127.0.0.1:4000","text=test&boundary.circle.lat=0&boundary.circle.lon=0&boundary.circle.radius=1")
  })
})